"""Estimate fold-level permutation and model-specific feature importance.

Use DEV cross-validation and saved full-model configurations. Aggregate
transformed-column importance back to the source clinical predictors.
Permutation importance uses average precision as its scoring metric."""
...
TARGET_SCHEMES = ["CatBoost+S0", "CatBoost+S1", "CatBoost+S2", "CatBoost+S3"]
'\n    "CatBoost+S0",\n    "CatBoost+S1",\n    "CatBoost+S2",\n    "CatBoost+S3",\n    "ExtraTrees+S0",\n    "ExtraTrees+S1",\n    "ExtraTrees+S2",\n    "ExtraTrees+S3",\n    "XGBoost+S3",\n'
OUT_ROOT = Path("outputs/feature_importance")
...
def aggregate_transformed_importance_to_raw(
    ...
    """Sum transformed-column importance for each original clinical predictor."""
    values = np.asarray(values, dtype=float)
    if len(values) != len(transformed_feature_names):
        raise ValueError(
            "Length mismatch between values and transformed_feature_names."
        )
...
def extract_catboost_internal_importance(
    ...
    pool_val = Pool(
        data=X_val_trans, label=y_val, feature_names=transformed_feature_names
    )
...
def extract_xgboost_gain_importance(
    clf, transformed_feature_names: List[str], raw_feature_names: List[str]
) -> Dict[str, float]:
    """
...
def make_summary_table(
    foldwise_df: pd.DataFrame, scheme_name: str, model_name: str, strategy: str
) -> pd.DataFrame:
    """Aggregate fold-level importance into one summary row per predictor."""
    ...
    out = out.sort_values(
        by=["PI_mean", "PI_median", "feature"], ascending=[False, False, True]
    ).reset_index(drop=True)
    num_cols = [
        c for c in out.columns if c not in ["Scheme", "Model", "Strategy", "feature"]
    ]
    for c in num_cols:
