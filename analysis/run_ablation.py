"""Compare feature-selection strategies in an exploratory ablation analysis.

Evaluate PI-only, PVC-only, LFC-only, consensus, and stability-filter ablation
with fixed CatBoost parameters. The 600-iteration ablation configuration
differs from the final retuned model and is not used for final-model NRI/IDI."""
...
SELECTED_FEATURES_CSV = Path(
    f"outputs/feature_selection/{TARGET_SCHEME}/selected_features_{SCHEME_TYPE}.csv"
)
BEST_CONFIG_JSON = Path("outputs/model_selection/best_config.json")
OUT_DIR = Path("outputs/ablation")
OUT_DIR.mkdir(parents=True, exist_ok=True)
FIXED_PARAMS = {"depth": 3, "learning_rate": 0.03, "iterations": 600}
K_VALUES = [2, 4, 6]
...
def get_topk_features_for_strategy(
    strategy: str, k: int, contrib_df: pd.DataFrame, selected_df: pd.DataFrame
) -> List[str]:
    """
...
def main():
    fix_seed(SEED)
    print("=" * 70)
    print("  Exploratory feature-selection ablation")
    print(
        f"  CatBoost+S0 | Fixed parameters: depth=3, learning_rate=0.03, iterations=600"
    )
    print(f"  Subset sizes: {K_VALUES}")
    print("=" * 70)
    if BEST_CONFIG_JSON.exists():
        with open(BEST_CONFIG_JSON, "r") as f:
            cfg = json.load(f)
        print(f"\n[INFO] Fixed parameter source: {BEST_CONFIG_JSON}")
        print(
            f"  depth={cfg['params']['depth']}, lr={cfg['params']['learning_rate']}, iter={cfg['params']['iterations']}"
        )
    else:
        print(
            f"\n[WARN] File not found: {BEST_CONFIG_JSON}; using the default fixed parameters"
        )
    print(f"\n[1/5] Loading consensus and screening files...")
    if not CONTRIBUTION_CSV.exists():
        raise FileNotFoundError(f"Missing: {CONTRIBUTION_CSV}")
    contrib_df = pd.read_csv(CONTRIBUTION_CSV)
    print(f"  Consensus file after stability screening: {len(contrib_df)} predictors")
    ...
    print(f"  Feature screening file: {len(selected_df)} predictors")
    print(f"\n[2/5] Loading data...")
    ...
    print(f"\n[3/5] Selected predictors by strategy:")
    strategies = ["PI-only", "PVC-only", "LFC-only", "Consensus", "SkipCV"]
    strategy_features_cache = {}
    ...
    print(
        f"\n[4/5] Predictor types for ablation, n={len(all_used_features)}): continuous={len(cont_cols)} | binary={len(bin_cols)} | categorical={len(cat_cols)}"
    )
    preprocessor = build_preprocessor(cont_cols, bin_cols, cat_cols)
    df_idx = df.set_index("ID", drop=False)
    print(
        f"\n[5/5] Running ablation ({len(strategies)} strategies by {len(K_VALUES)} subset sizes)..."
    )
    all_results = []
    for strategy in strategies:
        for k in K_VALUES:
    ...
    print(f"\n{'=' * 70}")
    print(f"  Ablation results by strategy and subset size")
    print(f"{'=' * 70}")
    ...
    for k in K_VALUES:
        ...
        if not consensus_row.empty:
            ...
            print(f"\n  K={k}:")
            print(
                f"    Highest DEV average precision: {best_dev['Strategy']} ({best_dev['DEV_AUPRC']:.4f})"
            )
            print(
                f"    Highest held-out average precision: {best_indep['Strategy']} ({best_indep['INDEP_AUPRC']:.4f})"
            )
