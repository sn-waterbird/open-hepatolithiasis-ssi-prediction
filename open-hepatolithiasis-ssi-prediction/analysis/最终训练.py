# -*- coding: utf-8 -*-
"""
final_clinical_model_evaluation.py

作用：
1) 针对最终选定的冠军方案 (如 CatBoost+S1, TopK=3) 进行独立验证集最终评估。
2) 【新增】在精简后的特征集上重新做超参搜索（grid search），而非沿用全特征时的超参。
3) 基础模型(Base)从 OOF 汇总表中读取最佳阈值。
4) 进行模型重训，并同步构建 Platt (Sigmoid) 和 Isotonic 概率校准模型。
5) 为 Platt 和 Iso 校准模型自动扫描并计算它们自己专属的最佳阈值 (YoudenJ)。
6) 输出涵盖 18 项核心临床评估指标的详细结果表，并附加 Bootstrap 95% CI。
7) 输出最终独立验证集的预测概率 (用于画 Calibration Curve 和 DCA)。
"""
...
# =========================
# 最终临床模型配置 (EDIT HERE ONLY)
# =========================
TARGET_SCHEME = "CatBoost+S0"  # 冠军方案:"CatBoost+S0"
TARGET_TOPK = 4               # 最佳极简特征数:4
SCHEME_TYPE = "scheme2"        # scheme1 对应 median 的结果 , scheme2对应 mean 的结果

SEED = 42
N_FOLDS = 5
N_BOOTSTRAPS = 1000           # Bootstrap 重抽样次数

# =========================
# 超参搜索配置
# =========================
RETUNE_HYPERPARAMS = True     # 设为 True 则在精简特征集上重新调参；False 则沿用全特征最佳超参
# CatBoost 搜索网格（可根据需要调整）
CAT_DEPTHS = [3, 4, 6, 8]
CAT_LRS = [0.03, 0.1]
CAT_ITERS = [300, 600, 1000]
# 如果后续扩展其他模型，可在此添加对应网格
...
def confusion_metrics(y_true: np.ndarray, p: np.ndarray, thr: float) -> Dict[str, float]:
    y_true = np.asarray(y_true).astype(int)
    p = np.asarray(p).astype(float)
...
def find_best_threshold_youden(y_true: np.ndarray, p: np.ndarray, n_thresh: int = 201) -> Tuple[float, Dict[str, float]]:
    """扫描最佳阈值 (基于 Youden J 指数)"""
...
def generate_ci_report(y_true: np.ndarray, p: np.ndarray, thr: float, n_bootstraps: int = 1000, ci: int = 95, seed: int = 42) -> Dict[str, Any]:
    """生成包含点估计和 Bootstrap 95% CI 的格式化字典"""
    # 1. 计算点估计值
    ...
    print(f"      [INFO] 正在进行 {n_bootstraps} 次 Bootstrap 抽样计算 95% CI...")
    for _ in range(n_bootstraps):
        idx = np.random.choice(n_samples, n_samples, replace=True)
        y_b = y_true[idx]
        p_b = p[idx]
        
        # 避免抽样中只有一个类别导致无法计算 AUC
...
def build_preprocessor(continuous_cols: List[str], binary_cols: List[str], cat_cols: List[str]) -> ColumnTransformer:
    transformers = []
    if continuous_cols:
        transformers.append(("cont", Pipeline([("imp", SimpleImputer(strategy="median", add_indicator=True))]), continuous_cols))
    if binary_cols:
        transformers.append(("bin", Pipeline([("imp", SimpleImputer(strategy="most_frequent"))]), binary_cols))
    if cat_cols:
...
def build_estimator(model_name: str, params: Dict[str, Any], strategy: str, pos_weight: float, seed: int):
    ...
    raise ValueError(f"Only CatBoost configured in this simplified version. Modify if needed.")
...
# =========================
# Hyperparameter grid for retuning on reduced feature set
# =========================
def build_catboost_grid():
    """CatBoost 超参搜索网格（在精简特征集上使用）"""
    grid = []
    for depth in CAT_DEPTHS:
        for lr in CAT_LRS:
            for it in CAT_ITERS:
                grid.append({"depth": depth, "learning_rate": lr, "iterations": it})
    return grid  # 4 * 2 * 3 = 24 种组合
...
def cv_evaluate_params(
    ...
    """在精简特征集上做 5 折 CV，返回平均 AUPRC 等指标"""
    ...
    if len(auprcs) == 0:
        return {"mean_AUPRC": -1.0, "mean_AUROC": -1.0, "mean_Brier": 1.0, "mean_LogLoss": 1.0}
...
def retune_hyperparams(
    ...
    """在精简特征集上做 grid search，返回最佳超参"""
    print("\n" + "=" * 60)
    print(f"[RETUNE] 在精简特征集 ({len(feature_cols)}个特征) 上重新搜索超参...")
    print("=" * 60)
    ...
    for params in grid:
        metrics = cv_evaluate_params(
            model_name, params, strategy, preprocessor,
            feature_cols, df_idx, sp_dev, y_col, folds, seed
        )
        results.append({**params, **metrics})
        print(f"  [CV] {params} -> AUPRC={metrics['mean_AUPRC']:.4f}")
    
    result_df = pd.DataFrame(results)
    # 按 mean_AUPRC 降序，mean_Brier 升序排序
    ...
    print(f"\n[RETUNE] 最佳超参: {best_params}")
    print(f"[RETUNE] 最佳 CV AUPRC: {best['mean_AUPRC']:.4f}")
...
def cv_get_base_oof(
    ...
    """在 DEV 上做 5 折 CV，返回 OOF 预测概率（用于阈值确定，防止数据泄露）"""
    n = len(sp_dev)
    ...
    for f in folds:
        tr_ids = sp_dev.loc[sp_dev["fold_id"] != f, "ID"].tolist()
        va_ids = sp_dev.loc[sp_dev["fold_id"] == f, "ID"].tolist()
        # 使用位置索引（np.where）而非标签索引（.index），确保与 oof 数组对齐
...
def main():
    fix_seed(SEED)
    print(f"========== 临床部署最终验证 (附带 Bootstrap 95% CI) ==========")
    print(f"目标方案: {TARGET_SCHEME} | TopK: {TARGET_TOPK}")
    ...
    print(f"[INFO] 提取的核心特征 ({len(features)}个): {features}")
    ...
    df = df.merge(sp[["ID", "split", "Infection"]], on="ID", how="inner", suffixes=("", "_split"))
    ...
    if RETUNE_HYPERPARAMS:
        ...
        print(f"\n[INFO] 使用精简特征集重新调参后的最佳超参: {best_params}")
        print(f"[INFO] 全特征时的原超参: {params_old}")
        params = best_params
    else:
        params = params_old
        print(f"[INFO] 沿用全特征时的超参: {params}")

    # 5. 通过 DEV OOF 确定 Base 模型最佳阈值，再训练最终模型
    print("\n[RUN] 正在获取 Base 模型 DEV OOF 预测以确定阈值...")
    p_base_oof = cv_get_base_oof(
        model_name=model_name, params=params, strategy=strategy,
        preprocessor=preprocessor, feature_cols=features,
        df_idx=df_idx, sp_dev=sp_dev_with_fold, y_col=y_col,
        folds=folds, seed=SEED,
    )
    best_thr_base, _ = find_best_threshold_youden(ytr, p_base_oof)
    print(f"  [INFO] Base 模型 DEV OOF 最佳阈值: {best_thr_base:.6f}")

    print("[RUN] 正在训练最终基础模型 (Base Model)...")
    ...
    # 6. 训练 Platt (Sigmoid) 校准模型 (5折CV防止过拟合)
    print("[RUN] 正在训练 Platt (Sigmoid) 校准模型...")
    ...
    # 打印前几列重要指标供终端查看
    print_cols = ["Model_Status", "AUROC", "AUPRC", "Brier_Score", "Thr_used", "Sensitivity", "Specificity"]
    print("\n" + report_df[print_cols].to_string(index=False))
    
    report_path = OUTPUT_ROOT / f"Final_Metrics_with_CI_{TARGET_SCHEME}_Top{TARGET_TOPK}.csv"
    report_df.to_csv(report_path, index=False, encoding="utf-8-sig")

    # 9. 导出预测结果表 (画校准曲线和DCA的核心资产)
    ...
    print(f"\n[SUCCESS] 全部完成！")
    print(f"-> 包含全套18项临床指标(附 Bootstrap 95% CI)的最终总结表已保存至: {report_path}")
    print(f"-> DCA与校准曲线专用预测表已保存至: {pred_path}")
