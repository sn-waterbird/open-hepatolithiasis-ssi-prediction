# -*- coding: utf-8 -*-
"""
feature_selection_by_two_rules_cv_thresholds_debug.py

作用：
1) 读取总表 all_schemes_feature_importance_summary.csv
2) 对每个方案按两套规则做门槛筛选
3) 在每个方案目录下分别保存：
   - selected_features_scheme1.csv
   - selected_features_scheme2.csv
   - selected_features_scheme1_kept_only.csv
   - selected_features_scheme2_kept_only.csv
   - selection_summary.csv
4) 在总输出目录下汇总保存：
   - all_schemes_selected_features_scheme1.csv
   - all_schemes_selected_features_scheme2.csv
   - all_schemes_selected_features_scheme1_kept_only.csv
   - all_schemes_selected_features_scheme2_kept_only.csv
   - all_schemes_selection_summary.csv

规则说明
-----------------------------------
规则1（主分析，稳健）：
- 中心判断统一用 median
- 稳定性统一用 CV = SD / (|mean| + eps)

PI:
    PI_median > 0
    and PI_CV <= PI_CV_THRESH

CatBoost:
    PVC_median > 0 and LFC_median > 0
    and PVC_CV <= PVC_CV_THRESH
    and LFC_CV <= LFC_CV_THRESH

XGBoost:
    GAIN_median > 0
    and GAIN_CV <= GAIN_CV_THRESH

ExtraTrees:
    ET_IMP_median > 0
    and ET_IMP_CV <= ET_CV_THRESH


规则2（敏感性分析）：
- 中心判断统一用 mean
- 稳定性仍统一用 CV = SD / (|mean| + eps)
...
注意：
- CV 的定义固定为：std / (abs(mean) + eps)
- 这版增加了 debug 列，方便排查到底是哪一步没通过
"""
...
def ensure_required_base_cols(df: pd.DataFrame):
    required = ["Scheme", "Model", "Strategy", "feature", "PI_mean", "PI_median", "PI_std"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        raise ValueError(f"Missing required columns: {missing}")


def compute_cv(std_s: pd.Series, mean_s: pd.Series, eps: float = 1e-8) -> pd.Series:
    """
    CV = std / (abs(mean) + eps)
    """
...
def apply_scheme1(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 初始化 debug 列，避免不同模型 concat 出现列缺失混乱
    for c in [
        "pass_PVC_center", "pass_LFC_center", "pass_PVC_stability", "pass_LFC_stability",
        "pass_GAIN_center", "pass_GAIN_stability",
        "pass_ET_center", "pass_ET_stability"
    ]:
        out[c] = np.nan
    ...
    if model == "CatBoost":
        ...
        out["pass_model_internal"] = (
            out["pass_GAIN_center"] &
            out["pass_GAIN_stability"]
        )
...
def select_output_columns_scheme1(df: pd.DataFrame) -> pd.DataFrame:
    base_cols = [
        ...
        "PI_mean", "PI_median", "PI_std", "PI_CV"
    ]
    extra_cols = []
    ...
    if model == "CatBoost":
        extra_cols += [
            "PVC_mean", "PVC_median", "PVC_std", "PVC_CV",
            "LFC_mean", "LFC_median", "LFC_std", "LFC_CV"
        ]
    elif model == "XGBoost":
        extra_cols += [
            "GAIN_mean", "GAIN_median", "GAIN_std", "GAIN_CV"
        ]
    elif model == "ExtraTrees":
        extra_cols += [
            "ET_IMP_mean", "ET_IMP_median", "ET_IMP_std", "ET_IMP_CV"
        ]

    flag_cols = [
        "pass_PI_center", "pass_PI_stability", "pass_model_internal", "keep_scheme1",
        "pass_PVC_center", "pass_LFC_center", "pass_PVC_stability", "pass_LFC_stability",
        "pass_GAIN_center", "pass_GAIN_stability",
        "pass_ET_center", "pass_ET_stability",
        "PI_CV_THRESH_USED", "PVC_CV_THRESH_USED", "LFC_CV_THRESH_USED",
        "GAIN_CV_THRESH_USED", "ET_CV_THRESH_USED"
    ]
...
def select_output_columns_scheme2(df: pd.DataFrame) -> pd.DataFrame:
    ...
    flag_cols = [
        "pass_PI_center", "pass_PI_stability", "pass_model_internal", "keep_scheme2",
        "pass_PVC_center", "pass_LFC_center", "pass_PVC_stability", "pass_LFC_stability",
        "pass_GAIN_center", "pass_GAIN_stability",
        "pass_ET_center", "pass_ET_stability",
        "PI_CV_THRESH_USED", "PVC_CV_THRESH_USED", "LFC_CV_THRESH_USED",
...
def build_kept_only_summary(df: pd.DataFrame, keep_col: str, center_col: str) -> pd.DataFrame:
    """
    只提取保留下来的特征，便于后续直接训练。
    """
...
def build_scheme_summary(scheme: str, model: str, strategy: str, out1: pd.DataFrame, out2: pd.DataFrame) -> pd.DataFrame:
    row = {
        "Scheme": scheme,
        "Model": model,
        "Strategy": strategy,
        "n_total_features": int(len(out1)),
        "n_keep_scheme1": int(out1["keep_scheme1"].sum()) if "keep_scheme1" in out1.columns else 0,
        "n_keep_scheme2": int(out2["keep_scheme2"].sum()) if "keep_scheme2" in out2.columns else 0,
...
def main():
    ...
    numeric_candidates = [
        "PI_mean", "PI_median", "PI_std",
        "PVC_mean", "PVC_median", "PVC_std",
        "LFC_mean", "LFC_median", "LFC_std",
        "GAIN_mean", "GAIN_median", "GAIN_std",
        "ET_IMP_mean", "ET_IMP_median", "ET_IMP_std",
    ]
    ...
    print(
        "[INFO] CV thresholds | "
        f"PI={PI_CV_THRESH}, PVC={PVC_CV_THRESH}, LFC={LFC_CV_THRESH}, "
        f"GAIN={GAIN_CV_THRESH}, ET={ET_CV_THRESH}, EPS={EPS}"
    )

    for scheme in scheme_list:
        sub = all_df[all_df["Scheme"].astype(str) == scheme].copy().reset_index(drop=True)
        if sub.empty:
            continue
        ...
        kept1 = build_kept_only_summary(out1, keep_col="keep_scheme1", center_col="PI_median")
        ...
        kept2 = build_kept_only_summary(out2, keep_col="keep_scheme2", center_col="PI_mean")
    ...
    if len(all_scheme1_kept) > 0:
        ...
        path_all_1_kept = IMPORTANCE_ROOT / "all_schemes_selected_features_scheme1_kept_only.csv"
    ...
    if len(all_scheme2_kept) > 0:
        ...
        path_all_2_kept = IMPORTANCE_ROOT / "all_schemes_selected_features_scheme2_kept_only.csv"
    ...
    if len(all_scheme_summary) > 0:
        ...
        all_scheme_summary_df.to_csv(path_all_summary, index=False, encoding="utf-8-sig")
