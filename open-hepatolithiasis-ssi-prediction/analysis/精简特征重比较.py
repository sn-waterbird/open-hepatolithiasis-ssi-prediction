...
作用：
  在 MCFS 选出的 TopK 特征上，固定超参后重新比较模型×策略组合：
    【方案B】CatBoost S0/S1/S2/S3 全部使用 CatBoost+S0 的全特征最优超参
            LR/S0、XGBoost/S0、ExtraTrees/S0 各自使用自己的全特征最优超参

  策略说明：
    - S0 (无处理) 用于跨模型比较 (LR / XGBoost / ExtraTrees)
    - S0~S3 用于 CatBoost 内部策略比较 (验证 S0 仍为最优)
    - 固定超参模式（FREEZE_HYPERPARAMS=True）：跳过网格搜索，使用全特征最优超参
      性能差异仅来自特征集变化（控制变量法），避免超参变化混淆消融实验结果

输出：
  精简特征重比较/
    ├── multi_scheme_comparison.csv      # 7 个 scheme 的 DEV+INDEP 完整对比
    ├── summary_comparison_vs_full.csv   # 核心论文表: TopK vs 全特征对比
    └── {scheme}/                        # 每个 scheme 的子目录
        ├── per_fold_metrics_raw.csv
        ├── dev_oof_predictions.csv
        ├── dev_oof_metrics_threshold_scan.csv
        ├── indep_predictions.csv
        ├── indep_confusion_matrix.csv
        ├── report_metrics_table.csv
        └── report_curves_roc_pr.csv
"""
...
#  消融实验：固定超参开关（方案B）
# True  = 使用全特征最优超参（控制变量法，性能差异仅来自特征集）
# False = 在 TopK 特征上重新网格搜索（辅助分析）
...
OUT_DIR = Path("精简特征重比较")
OUT_DIR.mkdir(parents=True, exist_ok=True)

FULL_FEATURE_SUMMARY = Path("训练全特征输出/multi_scheme_summary_dev_indep.csv")
SCHEME_PARAMS_CSV = Path("训练全特征/scheme_to_best_params.csv")

# ★ 所有 7 个方案统一重跑（包括 CatBoost+S0），不再单独读取最终训练.py 的输出
# 方案B规则：CatBoost 系全部使用 S0 的全特征最优超参；其他模型各用各自的
NEW_SCHEMES = [
    ("CatBoost", "S0"),   
    ("CatBoost", "S1"),
    ("CatBoost", "S2"),
...
# =========================
# Hyper-parameter Grids (same as 训练全特征.py)
# =========================
def lr_grid():
...
def et_grid():
    ...
    for ne in [300, 600, 1000]:
        for md in [None, 8, 12]:
            for msl in [1, 2]:
                grid.append({"n_estimators": ne, "max_depth": md, "min_samples_leaf": msl})
    return grid  # 18
...
def build_pipeline(preprocessor: ColumnTransformer, estimator, strategy: str, seed: int):
    if strategy in ("S2", "S3"):
        ...
        smote = SMOTE(sampling_strategy=ratio, k_neighbors=SMOTE_K_NEIGHBORS, random_state=seed)
        return ImbPipeline([("prep", preprocessor), ("smote", smote), ("clf", estimator)])
    else:
...
# =========================
# Frozen Hyperparams (方案B)
# =========================
def load_full_feature_best_params() -> Dict[str, Dict[str, Any]]:
    """
    从 训练全特征/scheme_to_best_params.csv 读取每个方案的全特征最优超参。
    
    方案B规则：
    - CatBoost S0/S1/S2/S3 全部统一使用 CatBoost+S0 的全特征最优超参
    - LR/S0、XGBoost/S0、ExtraTrees/S0 各用自己在全特征上的最优超参
    
    返回: { "CatBoost+S0": {"depth":3, "learning_rate":0.03, "iterations":600}, ... }
    """
...
# =========================
# Summary Table Builder (same as 训练全特征.py)
# =========================
def summarize_best_params_per_scheme(per_fold: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ok = per_fold[per_fold["status"] == "ok"].copy()
    if ok.empty:
        raise RuntimeError("All runs failed.")

    agg = (
        ok.groupby(["model", "strategy", "params"], as_index=False)
        .agg(
    ...
    summary_scheme_best.insert(
        0, "scheme",
        summary_scheme_best["model"].astype(str) + "+" + summary_scheme_best["strategy"].astype(str),
    )
    summary_scheme_best = summary_scheme_best.sort_values(
        by=["mean_AUPRC", "mean_AUROC", "mean_Brier"],
    ...
    scheme_best_params.insert(
        0, "scheme",
        scheme_best_params["model"].astype(str) + "+" + scheme_best_params["strategy"].astype(str),
    )
...
# =========================
# Core: Evaluate One Scheme on TopK Features
# =========================
def evaluate_one_scheme_on_topk(
    ...
    print(f"\n{'=' * 60}")
    print(f"[{scheme_name}] 在 {len(features)} 个特征上开始评估...")
    print(f"{'=' * 60}")

    out_dir.mkdir(parents=True, exist_ok=True)

    # --- Step 1: Get best params (frozen or grid search) ---
    if FREEZE_HYPERPARAMS:
        # ★ 方案B：使用全特征最优超参（固定超参，控制变量法）
        if full_best_params is None:
            raise ValueError("FREEZE_HYPERPARAMS=True 但未提供 full_best_params")
        if scheme_name not in full_best_params:
            raise KeyError(f"在 scheme_to_best_params.csv 中找不到 {scheme_name}")
        best_params = full_best_params[scheme_name]
        print(f"[{scheme_name}] ★ 固定超参模式: {best_params} (来自 {SCHEME_PARAMS_CSV})")

        # 固定超参模式下仍保存一条 per_fold 记录（用于追溯）
        records = [{
            "model": model_name,
            "strategy": strategy,
        ...
        per_fold.to_csv(out_dir / "per_fold_metrics_raw.csv", index=False, encoding="utf-8-sig")
    else:
        # 调参模式：在 TopK 特征上重新网格搜索（保持原有行为）
        print(f"[{scheme_name}] Step 1/3: 网格搜索 5 折交叉验证 (TopK 特征)...")
        ...
        for params in grid:
            for f in folds:
                ...
                try:
                    ...
                    m = {"AUROC": np.nan, "AUPRC": np.nan, "Brier": np.nan, "LogLoss": np.nan}
                    status = "fail"
                    err = repr(e)[:300]
                records.append({
                    "model": model_name,
        ...
        print(f"[{scheme_name}] 最佳参数: {best_params}")

    # --- Step 2: DEV OOF with best params + Youden threshold ---
    print(f"[{scheme_name}] Step 2/3: DEV OOF 预测 + Youden 阈值...")
    oof_rows = []
    for f in folds:
        tr_ids = sp_dev.loc[sp_dev["fold_id"] != f, "ID"].tolist()
        va_ids = sp_dev.loc[sp_dev["fold_id"] == f, "ID"].tolist()
    ...
    print(f"[{scheme_name}] DEV OOF: AUPRC={dev_oof_basic['AUPRC']:.4f}, AUROC={dev_oof_basic['AUROC']:.4f}, Thr={best_thr:.4f}")

    # --- Step 3: Train on full DEV -> INDEP ---
    print(f"[{scheme_name}] Step 3/3: 全 DEV 训练 -> INDEP 评估...")
    ...
    est_final = build_estimator(model_name, best_params, strategy, pos_weight_full, SEED)
    ...
    for ds_name, y_ds, p_ds, cm_ds in [
        ...
        report_rows.append({
            "Scheme": scheme_name, "Dataset": ds_name,
    ...
    print(f"[{scheme_name}] 完成. INDEP: AUPRC={indep_basic['AUPRC']:.4f}, AUROC={indep_basic['AUROC']:.4f}")
...
# =========================
# (已废弃) CatBoost+S0 不再单独读取最终训练.py 输出
# S0 已加入 NEW_SCHEMES 统一重跑，使用 CatBoost+S0 全特征最优超参
# =========================
...
def main():
    fix_seed(SEED)
    print("=" * 70)
    print("  精简特征重比较 — 在 MCFS 选出的 TopK 特征上重比较模型")
    print(f"  TopK = {TARGET_TOPK}, 方案 = {TARGET_SCHEME}, 类型 = {SCHEME_TYPE}")
    if FREEZE_HYPERPARAMS:
        print(f"  ★ 方案B: 固定超参模式（控制变量法）")
    else:
        print(f"  ★ 调参模式: 在 TopK 特征上重新网格搜索")
    print("=" * 70)

    # 0. Load full-feature best params (if frozen mode)
    full_best_params = None
    if FREEZE_HYPERPARAMS:
        print(f"\n[0/5] 加载全特征最优超参: {SCHEME_PARAMS_CSV}")
        if not SCHEME_PARAMS_CSV.exists():
            raise FileNotFoundError(f"Missing: {SCHEME_PARAMS_CSV}（请先运行 训练全特征.py）")
        full_best_params = load_full_feature_best_params()

    # 1. Load contribution -> TopK features
    print(f"\n[1/5] 读取贡献度文件: {CONTRIBUTION_CSV}")
    ...
    print(f"  Top{TARGET_TOPK} 特征: {features}")
    ...
    # 3. Feature types (restricted to TopK features)
    ...
    # 4. Run all 7 schemes (CatBoost+S0/S1/S2/S3 + LR/S0 + XGBoost/S0 + ExtraTrees/S0)
    print(f"\n[4/5] 运行 {len(NEW_SCHEMES)} 个方案...")
    all_summary_rows = []
    ...
    # Print
    print("\n" + "-" * 70)
    print(f"  精简特征集 (Top{TARGET_TOPK}) 模型对比 (按 INDEP_AUPRC 排序)")
    print("-" * 70)
    print_cols = ["rank", "Scheme", "n_features", "DEV_AUPRC", "INDEP_AUPRC", "INDEP_AUROC"]
    ...
    # Compare with full-feature results
    print(f"\n  -> 与全特征结果对比...")
    if FULL_FEATURE_SUMMARY.exists():
        ...
        for _, row in summary_df.iterrows():
            ...
            if not full_match.empty:
                ...
                delta = (
                    float(row["INDEP_AUPRC"]) - safe_float(fr.get("INDEP_AUPRC", float("nan")))
                    if (row["INDEP_AUPRC"] == row["INDEP_AUPRC"] and
                        str(fr.get("INDEP_AUPRC", "nan")) != "nan")
                    else float("nan")
                )
                comparison_rows.append({
                    ...
                    "n_features_full": 63,
                    "TopK_DEV_AUPRC": safe_float(row.get("DEV_AUPRC", float("nan"))),
                    "TopK_INDEP_AUPRC": safe_float(row.get("INDEP_AUPRC", float("nan"))),
                    "TopK_INDEP_AUROC": safe_float(row.get("INDEP_AUROC", float("nan"))),
                    "Full_DEV_AUPRC": safe_float(fr.get("DEV_AUPRC", float("nan"))),
                    "Full_INDEP_AUPRC": safe_float(fr.get("INDEP_AUPRC", float("nan"))),
                    "Full_INDEP_AUROC": safe_float(fr.get("INDEP_AUROC", float("nan"))),
                    "AUPRC_Delta": delta,
                })
            else:
                comparison_rows.append({
        ...
        comp_vs_full = comp_vs_full.sort_values(
            by=["TopK_INDEP_AUPRC", "TopK_INDEP_AUROC"],
            ascending=[False, False],
        ).reset_index(drop=True)
        ...
        print("\n" + "-" * 70)
        print(f"  Top{TARGET_TOPK} vs 全特征 (66 features) 对比")
        print("-" * 70)
        vs_cols = ["Scheme", "TopK_INDEP_AUPRC", "Full_INDEP_AUPRC",
                    "AUPRC_Delta", "TopK_INDEP_AUROC", "Full_INDEP_AUROC"]
        print(comp_vs_full[vs_cols].round(4).to_string(index=False))
    else:
        print(f"  [警告] 全特征结果文件不存在: {FULL_FEATURE_SUMMARY}，跳过对比。")
