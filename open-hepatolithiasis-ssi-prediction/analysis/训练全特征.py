# -*- coding: utf-8 -*-
"""
stage3_cv_select_models.py

DEV-only 5-fold CV model selection (journal-safe, reproducible)
===============================================================
Inputs (fixed):
- DATA_CSV            : 开腹肝胆道结石手术SSI清洗.csv
- SPLIT_CSV           : splits/split_folds_full.csv   (ID, split, fold_id, Infection)
- FEATURE_TYPES_CSV   : 特征类别/feature_types_summary.csv     (feature, type)

Fixed preprocessing:
- Continuous : median imputation + missing indicator
- Binary     : mode imputation (keep 0/1)
- Categorical: mode imputation + one-hot encoding

Imbalance strategies:
S0: None
S1: Class weight (per fold training set): w_pos = Nneg/Npos
S2: SMOTE 1:3 (train fold only)
S3: SMOTE 1:2 (train fold only)

Models:
LR, CatBoost, XGBoost, ExtraTrees

Hyperparameter grids (moderately expanded; controlled):
- LR        : C in [0.001, 0.01, 0.1, 1, 10, 100]
- CatBoost  : depth in [3,4,6,8], lr in [0.03,0.1], iters in [300,600,1000]
- XGBoost   : max_depth in [2,3,4,5,6], lr in [0.03,0.1], n_estimators in [200,400,800]
- ExtraTrees: n_estimators in [300,600,1000], max_depth in [None,8,12], min_samples_leaf in [1,2]

Outputs (OUT_DIR fixed):
1) per_fold_metrics_raw.csv
   - every (model, strategy, params, fold) record
2) summary_bestparams_per_scheme.csv
   - for each scheme (model, strategy): pick ONE best hyperparam setting (by mean AUPRC)
   - show its mean/std metrics (across folds)
   - DOES NOT include params columns (clean table for paper / reporting)
3) best_config.json
   - global best (model+strategy+best params)

Notes:
- "scheme" means (model, strategy). You asked summary to only compare schemes.
- Selection criterion: primary mean AUPRC, tie-break by mean AUROC then mean Brier (lower better).
"""
...
# XGB fixed regularization knobs (keep stable for journal-safety)
XGB_SUBSAMPLE = 0.8
XGB_COLSAMPLE = 0.8
XGB_REG_LAMBDA = 1.0
...
def load_feature_types(ft_path: Path, df_columns: List[str], exclude: List[str]) -> Tuple[List[str], List[str], List[str]]:
    ...
    # Any leftover columns not present in feature_types file -> continuous (conservative)
...
def summarize_best_params_per_scheme(per_fold: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if ok.empty:
        raise RuntimeError("All runs failed. Check per_fold_metrics_raw.csv -> error column.")

    # 1) aggregate per (model,strategy,params)
    agg = (
    ...
    # 2) for each (model,strategy), pick best params by ranking:
    # primary mean_AUPRC desc, tie mean_AUROC desc, tie mean_Brier asc
    agg = agg.sort_values(
    ...
    # 3) scheme-level clean table (drop params)
    ...
    # rank schemes globally for easy comparison (across all schemes)
    ...
    # drop params in final clean summary
    summary_clean = summary_scheme_best.drop(columns=["params"])

    # keep a mapping file for traceability (scheme -> best params json)
    scheme_best_params = best_params[["model", "strategy", "params"]].copy()
    scheme_best_params.insert(0, "scheme", scheme_best_params["model"].astype(str) + "+" + scheme_best_params["strategy"].astype(str))
...
def main():
    ...
    if not FEATURE_TYPES_CSV.exists():
        raise FileNotFoundError(f"Missing FEATURE_TYPES_CSV: {FEATURE_TYPES_CSV.resolve()}")
    ...
    # authoritative labels from split file
    df = df.merge(sp[["ID", "Infection"]], on="ID", how="inner",
    ...
    cont_cols, bin_cols, cat_cols = load_feature_types(FEATURE_TYPES_CSV, df.columns.tolist(), exclude=exclude_cols)
    ...
    # 2) scheme-level summary (best params selected internally; summary hides params)
    ...
    # 3) global best config
    best_row = scheme_best_params.merge(summary_clean[["scheme", "rank", "mean_AUPRC", "mean_AUROC", "mean_Brier", "mean_LogLoss"]], on="scheme", how="left")
    best_row = best_row.sort_values(by=["mean_AUPRC", "mean_AUROC", "mean_Brier"], ascending=[False, False, True]).iloc[0]
