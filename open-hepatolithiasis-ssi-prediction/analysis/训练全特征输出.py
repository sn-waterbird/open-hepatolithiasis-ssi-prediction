# -*- coding: utf-8 -*-
"""
stage4_multi_scheme_retrain_oof_threshold_indep_eval.py

多方案通用版 stage4：
- 读取 训练全特征/scheme_to_best_params.csv
- 通过 TARGET_SCHEMES 指定要跑的多个方案
- 每个方案独立输出到：
    OUT_ROOT / <scheme_name> /

流程（每个方案）：
1) 从 scheme_to_best_params.csv 读取目标方案（如 CatBoost+S0）
2) DEV OOF predictions with fixed hyperparams (5-fold)
3) Determine threshold on DEV OOF by Youden's J
4) Retrain final model on ALL DEV
5) Evaluate on independent set (VAL / INDEP_TEST / TEST)

输出（每个方案文件夹）：
- selected_scheme_config.json
- dev_oof_predictions.csv
- dev_oof_fold_metrics.csv
- dev_oof_metrics_threshold_scan.csv
- dev_oof_threshold_youden.json
...
额外总汇总：
- OUT_ROOT / multi_scheme_summary_dev_indep.csv
"""
...
# Threshold scan resolution
N_THRESH = 201  # 0.000 .. 1.000 step=0.005
...
def detect_indep_split(sp: pd.DataFrame) -> str:
    ...
    for cand in ["VAL", "INDEP_TEST", "TEST"]:
        if cand in splits:
            return cand
    raise ValueError(f"Cannot find independent split among {splits}. Expected one of VAL/INDEP_TEST/TEST.")


# =========================
# Core runner for one scheme
# =========================
def run_one_scheme(
    target_scheme: str,
    data_csv: Path,
    ...
    for f in folds:
        ...
        print(f"[OK] fold {f} done | val_AUPRC={bm['AUPRC']:.4f} val_AUROC={bm['AUROC']:.4f}")
    ...
    dev_oof_basic = safe_basic_metrics(dev_oof["y_true"].values, dev_oof["oof_prob"].values)

    # =========================
    # 2) Threshold by Youden on DEV OOF
    # =========================
    print("\n" + "=" * 60)
    ...
    best_thr, scan_df = find_best_threshold_youden(
        dev_oof["y_true"].values,
        dev_oof["oof_prob"].values,
        n_thresh=N_THRESH
    )
    ...
    thr_info = {
        ...
        "dev_oof_at_threshold": {k: float(v) if v == v else None for k, v in dev_oof_cm.items()},
        "model": model_name,
        "strategy": strategy,
        "params": params,
        "seed": int(SEED),
    ...
    thr_path.write_text(json.dumps(thr_info, ensure_ascii=False, indent=2), encoding="utf-8")
    print("[SAVE] Threshold info:", thr_path)
    print(
        f"[INFO] Best threshold = {best_thr:.4f} | "
        f"DEV-OOF YoudenJ={dev_oof_cm['YoudenJ']:.4f} "
        f"Sens={dev_oof_cm['Sensitivity']:.4f} "
        f"Spec={dev_oof_cm['Specificity']:.4f}"
    )

    # =========================
    # 3) Retrain on ALL DEV and evaluate on independent set
    # =========================
    print("\n" + "=" * 60)
    ...
    cm_tbl = pd.DataFrame([{
        "TN": indep_cm["TN"], "FP": indep_cm["FP"], "FN": indep_cm["FN"], "TP": indep_cm["TP"]
    }])
    ...
    indep_metrics_path.write_text(json.dumps(indep_metrics, ensure_ascii=False, indent=2), encoding="utf-8")
    print("[SAVE] Independent metrics:", indep_metrics_path)

    print(
        f"[INFO] {indep_split} threshold-free: "
        f"AUPRC={indep_basic['AUPRC']:.4f} "
        f"AUROC={indep_basic['AUROC']:.4f} "
        f"Brier={indep_basic['Brier']:.4f} "
        f"LogLoss={indep_basic['LogLoss']:.4f}"
    )
    print(
        f"[INFO] {indep_split} at thr={best_thr:.4f}: "
        f"Sens={indep_cm['Sensitivity']:.4f} "
        f"Spec={indep_cm['Specificity']:.4f} "
        f"PPV={indep_cm['PPV']:.4f} "
        f"NPV={indep_cm['NPV']:.4f} "
        f"F1={indep_cm['F1']:.4f}"
    )

    # =========================
    # 4) Readable report table
    # =========================
    print("\n" + "=" * 60)
    ...
    report_rows.append({
        ...
        "Sensitivity": float(dev_oof_cm["Sensitivity"]) if dev_oof_cm["Sensitivity"] == dev_oof_cm["Sensitivity"] else np.nan,
        "Specificity": float(dev_oof_cm["Specificity"]) if dev_oof_cm["Specificity"] == dev_oof_cm["Specificity"] else np.nan,
        "PPV": float(dev_oof_cm["PPV"]) if dev_oof_cm["PPV"] == dev_oof_cm["PPV"] else np.nan,
        "NPV": float(dev_oof_cm["NPV"]) if dev_oof_cm["NPV"] == dev_oof_cm["NPV"] else np.nan,
        "F1": float(dev_oof_cm["F1"]) if dev_oof_cm["F1"] == dev_oof_cm["F1"] else np.nan,
        "Accuracy": float(dev_oof_cm["Accuracy"]) if dev_oof_cm["Accuracy"] == dev_oof_cm["Accuracy"] else np.nan,
        "BalancedAcc": float(dev_oof_cm["BalancedAcc"]) if dev_oof_cm["BalancedAcc"] == dev_oof_cm["BalancedAcc"] else np.nan,
        "YoudenJ": float(dev_oof_cm["YoudenJ"]) if dev_oof_cm["YoudenJ"] == dev_oof_cm["YoudenJ"] else np.nan,
    })
    ...
    report_rows.append({
        ...
        "Threshold": float(best_thr),
        "Sensitivity": float(indep_cm["Sensitivity"]) if indep_cm["Sensitivity"] == indep_cm["Sensitivity"] else np.nan,
        "Specificity": float(indep_cm["Specificity"]) if indep_cm["Specificity"] == indep_cm["Specificity"] else np.nan,
        "PPV": float(indep_cm["PPV"]) if indep_cm["PPV"] == indep_cm["PPV"] else np.nan,
        "NPV": float(indep_cm["NPV"]) if indep_cm["NPV"] == indep_cm["NPV"] else np.nan,
        "F1": float(indep_cm["F1"]) if indep_cm["F1"] == indep_cm["F1"] else np.nan,
        "Accuracy": float(indep_cm["Accuracy"]) if indep_cm["Accuracy"] == indep_cm["Accuracy"] else np.nan,
        "BalancedAcc": float(indep_cm["BalancedAcc"]) if indep_cm["BalancedAcc"] == indep_cm["BalancedAcc"] else np.nan,
        "YoudenJ": float(indep_cm["YoudenJ"]) if indep_cm["YoudenJ"] == indep_cm["YoudenJ"] else np.nan,
    })
    ...
    # return summary row for global aggregation
    return {
        "Scheme": scheme_name,
        "Model": model_name,
        "Strategy": strategy,
        ...
        "DEV_Sensitivity": float(dev_oof_cm["Sensitivity"]) if dev_oof_cm["Sensitivity"] == dev_oof_cm["Sensitivity"] else np.nan,
        "DEV_Specificity": float(dev_oof_cm["Specificity"]) if dev_oof_cm["Specificity"] == dev_oof_cm["Specificity"] else np.nan,
        "DEV_PPV": float(dev_oof_cm["PPV"]) if dev_oof_cm["PPV"] == dev_oof_cm["PPV"] else np.nan,
        "DEV_NPV": float(dev_oof_cm["NPV"]) if dev_oof_cm["NPV"] == dev_oof_cm["NPV"] else np.nan,
        "DEV_F1": float(dev_oof_cm["F1"]) if dev_oof_cm["F1"] == dev_oof_cm["F1"] else np.nan,
        "DEV_YoudenJ": float(dev_oof_cm["YoudenJ"]) if dev_oof_cm["YoudenJ"] == dev_oof_cm["YoudenJ"] else np.nan,
        ...
        "INDEP_Sensitivity": float(indep_cm["Sensitivity"]) if indep_cm["Sensitivity"] == indep_cm["Sensitivity"] else np.nan,
        "INDEP_Specificity": float(indep_cm["Specificity"]) if indep_cm["Specificity"] == indep_cm["Specificity"] else np.nan,
        "INDEP_PPV": float(indep_cm["PPV"]) if indep_cm["PPV"] == indep_cm["PPV"] else np.nan,
        "INDEP_NPV": float(indep_cm["NPV"]) if indep_cm["NPV"] == indep_cm["NPV"] else np.nan,
        "INDEP_F1": float(indep_cm["F1"]) if indep_cm["F1"] == indep_cm["F1"] else np.nan,
        "INDEP_YoudenJ": float(indep_cm["YoudenJ"]) if indep_cm["YoudenJ"] == indep_cm["YoudenJ"] else np.nan,
    }
...
def main():
    ...
    if len(summary_rows) > 0:
        ...
        metric_cols = [c for c in summary_df.columns if c not in ["Scheme", "Model", "Strategy", "INDEP_Split"]]
        for c in metric_cols:
