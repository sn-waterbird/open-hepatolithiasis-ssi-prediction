# -*- coding: utf-8 -*-
"""
topk_retrain_scan_metrics_only.py

作用：
1) 遍历“特征筛选/”下所有方案文件夹
2) 分别读取：
   - contribution_scheme1.csv  （PI 用 median 的那套）
   - contribution_scheme2.csv  （PI 用 mean 的那套）
3) 按贡献排序，做 top-1, top-2, ..., top-K 特征训练
4) 每个 top-k：
   - DEV 5-fold OOF
   - DEV OOF 阈值扫描（YoudenJ）
   - ALL DEV 重训
   - INDEP / VAL 评估
5) 只输出指标表，不保存逐例预测
6) 不显示 SelectedFeatures 列，便于读表
7) [新增] 自动将结果按 INDEP_AUPRC 降序等规则排序，并输出 排序median.csv 和 排序mean.csv

输出：
- 特征筛选/<scheme>/topk_metrics_scheme1.csv
- 特征筛选/<scheme>/topk_metrics_scheme2.csv
- 遍历训练/all_schemes_topk_metrics_scheme1.csv
- 遍历训练/all_schemes_topk_metrics_scheme2.csv
- 遍历训练/排序median.csv
- 遍历训练/排序mean.csv
"""
...
OUTPUT_ROOT = Path("遍历训练")
OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)

MAX_TOPK = 20
N_THRESH = 201  # threshold scan on [0,1]
EPS = 1e-15
...
def build_preprocessor(continuous_cols: List[str], binary_cols: List[str], cat_cols: List[str]) -> ColumnTransformer:
    ...
    if continuous_cols:
        transformers.append((
            "cont",
            Pipeline([("imp", SimpleImputer(strategy="median", add_indicator=True))]),
            continuous_cols
        ))
...
def read_contribution_file(path: Path) -> pd.DataFrame:
    ...
    df = df.sort_values(["Rank", "Contribution_pct"], ascending=[True, False]).reset_index(drop=True)
    return df
...
def evaluate_topk_for_one_scheme(
    ...
    cont_all, bin_all, cat_all = load_feature_types(feature_types_csv, df.columns.tolist(), exclude=exclude_cols)
    ...
    for k in range(1, topk_max + 1):
        ...
        best_thr, best_thr_row = find_best_threshold_youden(y_dev, p_dev, n_thresh=N_THRESH)
        dev_at_thr = confusion_metrics(y_dev, p_dev, best_thr)

        # -------------------------
        # ALL DEV retrain -> INDEP
        # -------------------------
        ...
        rows.append({
            ...
            "Model": model_name,
            "Strategy": strategy,
            "FeatureSetType": scheme_type,   # scheme1 / scheme2
            "TopK": k,
        ...
        print(f"[OK] {scheme_name} | {scheme_type} | top-{k} done | INDEP_AUPRC={indep_basic['AUPRC']:.4f} INDEP_F1={indep_at_thr['F1']:.4f}")

    out = pd.DataFrame(rows)

    num_cols = [c for c in out.columns if c not in ["Scheme", "Model", "Strategy", "FeatureSetType", "INDEP_Split"]]
    for c in num_cols:
...
def main():
    ...
    if len(all_scheme1_outputs) > 0:
        ...
        save_sorted1 = OUTPUT_ROOT / "排序median.csv"
        all1_sorted.to_csv(save_sorted1, index=False, encoding="utf-8-sig")
        print(f"[SAVE] 排序已完成并保存至: {save_sorted1}")
    ...
    if len(all_scheme2_outputs) > 0:
        ...
        save_sorted2 = OUTPUT_ROOT / "排序mean.csv"
        all2_sorted.to_csv(save_sorted2, index=False, encoding="utf-8-sig")
        print(f"[SAVE] 排序已完成并保存至: {save_sorted2}")
